...
const SubIntro = () => {
  const { ref: titleRef, style: titleStyle } = useScrollAnimation("up", 0.4);
  const { ref: subTitleRef, style: subTitleStyle } = useScrollAnimation("up", 0.3);
  const { ref: assetRef, style: assetStyle } = useScrollAnimation("right", 0.4);
  ...
  return (
    <div className="w-full h-screen font-spline text-main-white">
      ...
      <div className="flex mobile:flex-col pad:flex-row pad:items-center ml-[clamp(30px,5.041vw,178px)] mobile:pt-[clamp(84px,23.3vh,130px)] pad:pt-[clamp(152px,19.79vh,180px)]">
        ...
        <div
          className="mobile:flex pad:block mobile:flex-row-reverse desktop:w-[clamp(359px,18.698vw,400px)] mobile:mt-[clamp(34px,5.313vh,50px)] pad:mt-[clamp(106px,13.8vh,140px)] desktop:mt-[clamp(75px,6.95vh,100px)] mobile:ml-[clamp(178px,55vw,768px)] pad:ml-[clamp(40px,4vw,260px)] desktop:ml-[clamp(30px,30vw,140px)]"
          ref={assetRef}
          style={assetStyle}
        >
          <img
            className="restart-animation mobile:w-[clamp(124px,34.44vw,150px)] pad:w-[clamp(221px,20.4vw,300px)] desktop:w-[clamp(359px,18.698vw,400px)]"
            src={subIntroAsset}
            alt=""
          />
        </div>
