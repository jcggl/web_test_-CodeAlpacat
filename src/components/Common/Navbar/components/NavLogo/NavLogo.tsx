...
const NavLogo = ({ location, checkActiveLink, isTechVision }:NavLogoType) => {
  ...
  return (
    ...
    >
      <img
        className="hidden pad:block pad:w-[clamp(144px,13.333vw,204px)] desktop:w-[clamp(204px,10.625vw,244.8px)] aspect-[221/65] transition-all ease-in-out duration-[300ms] hover:opacity-70"
        src={logo2}
        alt="굳갱랩스 로고"
        loading="lazy"
      />
      <img
        className="block pad:hidden w-[clamp(47.52px,13.2vw,65px)] aspect-[47.45/32] transition-all ease-in-out duration-[300ms] hover:opacity-70"
        src={logo}
        alt="굳갱랩스 로고"
        loading="lazy"
      />
